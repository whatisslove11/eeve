...
def merge_sentencepiece_tokenizers(
    ...
    old_tokenizer = AutoTokenizer.from_pretrained(
        path_or_name_to_old_tokenizer, use_fast=False
    )
    ...
    for piece in new_spm.pieces:
        ...
        if token not in existing_tokens:
            ...
            new_p.piece = token
            new_p.score = (
                piece.score + prev_min_score
                if piece.type == TOKEN_TYPES.NORMAL
                else 0.0
            )  # user defined tokens must have 0.0
    ...
    if nllb_additional_lang_codes:
        merged_tokenizer = type(old_tokenizer).from_pretrained(
            ...
            additional_special_tokens=sorted(
                FAIRSEQ_LANGUAGE_CODES + nllb_additional_lang_codes
            ),
        )
    else:
        merged_tokenizer = type(old_tokenizer).from_pretrained(
            old_tokenizer.name_or_path, vocab_file=NEW_SPM_NAME
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_path", type=str, required=True, help="Path to save the merged tokenizer"
    )
    parser.add_argument(
        "--tokenizer_name", type=str, required=True, help="Name for the new tokenizer"
    )
    parser.add_argument(
        "--lang_codes",
        nargs="*",
