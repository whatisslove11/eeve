...
from schema import EmbeddingObject, EmbeddingRequest, EmbeddingResponse
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    try:
        ...
        app.state.engine = AsyncEmbeddingEngine(
            model_name_or_path=model_name, engine=engine_type, device=device
        )
...
async def create_embeddings(request: EmbeddingRequest) -> EmbeddingResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Embedding processing error: {str(e)}"
        ) from e
...
async def health_check() -> dict:
    status = {"status": "ok", "model": "unknown", "device": "unknown", "ready": False}
