...
@hydra.main(config_path="/content/config", config_name="colab", version_base=None)
def train(cfg: DictConfig):
    ...
    if test_size_from_train := cfg.data.eval.get('test_size_from_train'):
        ...
        ds_dict = _load_dataset_from_path(path=cfg.data.train.path)
        train_dataset = ds_dict[cfg.data.train.split]
        eval_dataset = ds_dict[split_from_train_path]

    elif cfg.data.eval.get('path'):
        train_ds_dict = _load_dataset_from_path(path=cfg.data.train.path)
    ...
    eval_source_sentences = list(eval_dataset[cfg.data.source_column])
    eval_target_sentences = list(eval_dataset[cfg.data.target_column])

    if cfg.preprocessing.get('apply_function'):
        preprocess_func = hydra.utils.get_method(cfg.preprocessing.apply_function._target_)
        fn_kwargs = cfg.preprocessing.apply_function.get('fn_kwargs', {})
    ...
    evaluator = hydra.utils.instantiate(
        cfg.evaluator,
        source_sentences=eval_source_sentences,
        target_sentences=eval_target_sentences,
    )
